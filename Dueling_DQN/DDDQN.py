...
class DDDQN(nn.Module):
    ...
    def __init__(self, h, w, outputs):
        ...
        self.advantage = nn.Linear(linear_input_size,outputs)
...
policy_net = DDDQN(screen_height,screen_width,n_actions).to(device)
target_net = DDDQN(screen_height,screen_width,n_actions).to(device)
target_net.eval()
...
def optimize_model():
    ...
    next_action = torch.zeros(BATCH_SIZE, device=device,dtype=torch.long)
    #target_net을 이용하여 다음 state의 q value 계산.
    #만약에 terminal state이라면, 계산할 필요가 없음.
    next_action[non_final_mask] = policy_net(non_final_next_states).argmax(dim=1).detach()

    next_state_values[non_final_mask] = target_net(non_final_next_states)[[i for i in range(non_final_next_states.shape[0])],next_action[non_final_mask]].detach()
    # 기대 Q 값 계산
